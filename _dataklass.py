...
from typing import TypeVar, Type, dataclass_transform
...
@dataclass_transform()
def dataklass(cls: Type[_T]) -> Type[_T]:
    fields = all_hints(cls)
    nfields = len(fields)
    clsdict = vars(cls)
    if '__init__' not in clsdict: cls.__init__ = patch_args_and_attributes(make__init__(nfields), fields)
    if '__repr__' not in clsdict: cls.__repr__ = patch_attributes(make__repr__(nfields), fields, 2)
    if '__eq__' not in clsdict: cls.__eq__ = patch_attributes(make__eq__(nfields), fields, 1)
